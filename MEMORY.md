...
- [x] **REAL CARDS NOW ACTUALLY MOUNTED (2026-09-14):** Prior builds wired IssuingCardsSection but `App.tsx` STILL lazy-imported and rendered the demo `CardsSection` toy on the Cards tab (commit history diverged). Fixed: `App.tsx` lazy import `CardsSection` → `components/IssuingCardsSection`, mount dropped the unused `onIssueCard` prop. tsc 0, vite build OK, node --check OK, commit 747d868. `/api/issuing/status` live returns {available:false, reason:'no-stripe-key'} locally (no key in server/.env) — same self-flips-on design: set STRIPE_SECRET_KEY + activate Issuing in Render → cards issue immediately, no redeploy.
...
- [x] **USDT DEPOSIT RAIL + HOT-WALLET SIGNING FIXES (2026-09-14, pushed `47c5817`)**:
  1. **USDT rail written** — USDT was demo-data only; now real: shared xpub address pool (SAME address receives USDC+USDT on a network), `?token=usdc|usdt` on deposit-address, check scans BOTH tokens per network (6 getLogs rounds across base+polygon+ethereum), token-aware notes/notifications. Contracts verified: polygon `0xc2132…8e8F` (on-chain eth_call: symbol USD₮0, 6 dec), ethereum `0xdac17…1ec7` (tether.to official). **Base USDT0 skipped — no authoritative address found; do NOT add blindly** (test pins this).
  2. **CRITICAL FIX — hot-wallet EIP-155 signing was garbage**: `Array.concat` does NOT spread Uint8Array → every signed tx comma-garbage (broadcast would fail + auto-refund every time); also `keccak256()` returns Uint8Array and `String(u8)` / `hash.slice(-40)` produced garbage (address was last-40-BYTES not last-20). Fixed in `usdc-hotwallet.js`: plain-Array hexToBytes (accepts Array/TypedArray), address = bytesToHex(slice(-20)), `signTransaction` exported. **Pinned to the canonical EIP-155 spec vector byte-for-byte** (`__tests__/usdc.test.js` — key 0x4646…46 → known raw tx + derived address 0x9d8a62f6…). 14/14 usdc+usdt tests green.
  3. **(address, network) uniqueness migration** — global UNIQUE(address) silently dropped the 2nd network's row (demo exposed it: polygon row never stored) → deposits on that network never credited. ensureTables now migrates to UNIQUE(address, network) idempotently; INSERT uses ON CONFLICT (address, network).
  4. **polygon-rpc.com dead** (403 API-key required) → `polygon-bor-rpc.publicnode.com` in both usdc.js + usdc-hotwallet.js (env override still wins).
  5. Fixed broken cut-off-session work: api.ts duplicate `healthCheck` (syntax error), App.tsx spliced `?pay=` effect (addfunds effect was broken), VbPaySection unused import. All VbPay/Chain files now syntax-clean, tsc 0 errors, vite build OK (903KB).
  6. Verified live locally: USDT deposit-address returns real derived address + QR on ethereum+polygon (same address, token-aware note), check scans live chains and credits `[]` (fresh address, correct). Demo test-xpub addresses + scan-state rows DELETED from Neon after demo (never let a test-mnemonic address linger on the demo user — production must set the REAL `USDC_XPUB`).
  7. Browser MCP was down (no Chrome; Edge/Brave block `--load-extension` without manual load) — manual one-time: load+pin `C:\Users\cr7\.opencode-browser\extension` in a Chromium browser; broker runs via `node C:\Users\cr7\.opencode-browser\broker.cjs`.
  8. Needs in Render for USDT deposits: same `USDC_XPUB` (public only, no new env). Chain withdrawals additionally need `USDC_HOT_WALLET_KEY` + funded hot wallet.
