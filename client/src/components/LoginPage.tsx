...
import { ShieldCheck, Eye, EyeOff, ArrowRight, Lock, User, Zap, Diamond } from 'lucide-react';
import { api } from '../api';
...
export default function LoginPage({ onLogin, onSwitchToSignup }: Props) {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center p-4" style={{ background: '#050508' }}>
      {/* Ultra-luxury layered background with deep gold/diamond fields */}
        ...
        >
          ...
          <div className="relative z-10 space-y-5">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-white/35 mt-1.5">Access your private vault securely</p>
            </div>

            {/* Account Input */}
            <div className="space-y-1.5">
              <label className="text-[9px] text-white/35 tracking-[0.3em] font-semibold ml-1 block">ACCOUNT NUMBER</label>
              <div className="relative">
                <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${focused === 'account' ? 'text-amber-400' : 'text-white/30'}`}>
                ...
                <input
                  ...
                  style={{
                    background: focused === 'account' ? 'rgba(212,175,55,0.06)' : 'rgba(255,255,255,0.035)',
                    border: `1px solid ${focused === 'account' ? 'rgba(212,175,55,0.3)' : 'rgba(255,255,255,0.07)'}`,
                    boxShadow: focused === 'account' ? '0 0 20px rgba(212,175,55,0.05)' : 'none',
                  }}
                />
              </div>
            </div>
            ...
            {/* Password Input */}
            <div className="space-y-1.5">
              <label className="text-[9px] text-white/35 tracking-[0.3em] font-semibold ml-1 block">SECURITY CODE</label>
              <div className="relative">
            ...
            <div className="flex items-center justify-between text-xs">
              <label className="flex items-center gap-2 text-white/45 cursor-pointer hover:text-white/60 transition-colors">
                <input type="checkbox" className="w-3.5 h-3.5 rounded accent-amber-500" defaultChecked />
                Remember this device
              </label>
              <button className="text-amber-400/50 hover:text-amber-300 transition-colors">
                Reset code?
              </button>
            </div>
            ...
            >
              {loading ? (
                <><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}><Zap className="w-4 h-4" /></motion.div> Authenticating...</>
              ) : (
                <><ArrowRight className="w-4 h-4" /> Access Private Account</>
              )}
            </motion.button>
            ...
            <div className="text-center mt-4">
              ...
              >
                Don't have an account? <span className="text-amber-400/60 font-semibold">Create Account</span>
              </button>
            </div>
          </div>
        </motion.div>
